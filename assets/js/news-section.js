...
export function initNews() {
  ...
  const PUB_BASE = "https://docs.google.com/spreadsheets/d/e/2PACX-1vS4g1gNsiyY4k2A052LR7EaVUYFqrWGWNITOPqw5bGG_PTogiP84C44VQJpKn-HC2vxHin4fTy_puU0/pub";
  ...
  async function loadLatest4() {
    ...
    return latest.map((p) => ({
      ...
      tag:   p.tags?.[0] || "Update",
      date:  p.date  || "",
      cover: p.cover ? p.cover.split('?')[0] + '?auto=format&fit=crop&w=1200&q=70' : "",
      title: p.title || "",
      text:  p.excerpt || "",
    }));
  }
  ...
  function initBreathingAnimation() {
    ...
    function update() {
      ...
      wrappers.forEach((wrapper) => {
        ...
        // EXIT behind header
        // A card can only exit behind the header if it has reached its sticky position.
        // We detect this by checking if the wrapper's top is already above the viewport midpoint.
        const wrapperTop = wRect.top;
        const cardHasBeenSticky = wrapperTop < (vh * 0.5);
        const overlap = headerBottom - cardTop;

        if (overlap > 0 && cardHasBeenSticky) {
